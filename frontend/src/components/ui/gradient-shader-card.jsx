...
const GrainyGradient = forwardRef(function GrainyGradient(
  { ripples = [], onTimeUpdate },
  ref,
) {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < Math.min(activeRipples.length, 10); i += 1) {
      const ripple = activeRipples[i];
      const normalizedX = (ripple.x / 800) * 2 - 1;
      const normalizedY = 1 - (ripple.y / 600) * 2;
...
const fragmentShader = `
  ...
  float calculateRipples(vec2 uv) {
    ...
    for (int i = 0; i < rippleCount && i < 10; i++) {
      ...
      float ring = smoothstep(rippleRadius + rippleWidth, rippleRadius, dist) * smoothstep(rippleRadius - rippleWidth, rippleRadius, dist);
